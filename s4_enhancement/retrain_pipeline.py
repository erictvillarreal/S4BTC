...
# Umbrales minimos aceptables vs baseline conocido (ver
# s4_enhancement/RESULTADO_MATRIZ_Y_CAUSA_30DIAS.md, 26 Jun 2026)
MIN_WINRATE = 0.55       # baseline real: 0.6466-0.6472
MIN_SHARPE_PROXY_CAGR = 0.40   # baseline real: ~0.72
MAX_ACCEPTABLE_DD = -0.10      # baseline real: ~-0.02
...
def step_1_update_data():
    """Actualiza data/BTCUSDT.csv con velas nuevas desde OKX, detectando
    y rellenando huecos automaticamente si los hay."""
    ...
    df_new = get_historical_data("BTCUSDT", "1h", limit=600)
    ...
    # Deteccion temprana de huecos grandes ANTES de guardar — alerta si
    # el fetch normal no fue suficiente para cubrir todo el rango
    diffs = df_combined["open_time"].diff()
    big_gaps = diffs[diffs > pd.Timedelta(hours=48)]
    if len(big_gaps) > 0:
        _log(f"ADVERTENCIA: {len(big_gaps)} hueco(s) >48h detectado(s) tras el fetch normal.")
        _log("Esto requiere intervencion manual (fetch dirigido al hueco especifico).")
